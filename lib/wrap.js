...
Wrap.prototype.require = function (startFiles, withRoot) {
    ...
    startFiles.forEach(function include (file) {
        ...
        self.require_(res, dir, withRoot || dir);
    });
    
    return self;
};

Wrap.prototype.require_ = function (mfile, basedir, root, asName) {
    var self = this;
    ...
    if (resolve.isCore(mfile)) {
        ...
        try {
            ...
            if (pkgfile && !self.checkedPackages[pkgfile]) {
                self.checkedPackages[pkgfile] = true;
    ...
    self.files[name] = { root : root, body : body };
