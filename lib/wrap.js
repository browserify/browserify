...
Wrap.prototype.bundle = function () {
    ...
    var basedir = (function () {
        var required = Object.keys(self.files).filter(function (x) {
            return !self.files[x].target
        });
        var entries = Object.keys(self.entries).filter(function (x) {
            return !self.entries[x].target
        });
        var dir = commondir(required.concat(entries)) + '/';
    ...
    var src = []
        ...
        .concat(Object.keys(self.files).map(function (file) {
            ...
            return self.wrap(target, s.body)
        }))
        ...
        .concat(Object.keys(self.entries).map(function (file) {
            ...
            var target = s.target || removeLeading(file);
            return self.wrapEntry(target, s.body);
        }))
        .concat(this.appends)
        .join('\n')
    ;
...
Wrap.prototype.require = function (mfile, opts) {
    ...
    if (resolve.isCore(mfile)) {
        ...
        try {
            opts.file = self.resolver(mfile, opts.dirname);
        }
        catch (err) {
            throw new Error(
    ...
    var pkgfile = dirname + '/package.json';
    if (!mfile.match(/^(\.\.?)?\//)) {
        try {
