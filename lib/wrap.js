...
Wrap.prototype.require_ = function (mfile, opts) {
    ...
    if (resolve.isCore(mfile)) {
        ...
        try {
            var file = resolver(mfile, opts.basedir, self.extensions);
        }
        catch (err) {
    ...
    if (!opts.target) opts.target = makeTarget(file, opts.root);
    ...
    nub(required.strings).forEach(function (f) {
...
function resolver (file, basedir, extensions) {
