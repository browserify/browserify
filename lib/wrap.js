...
Wrap.prototype.bundle = function () {
    ...
    var src = []
        ...
        .concat(Object.keys(self.globals).map(function (global) {
...
/**
 * Bundle a module as global variable.
 *
 * Notice a special shortcut for Buffer:
 * 
 * b.global('Buffer')
 * //-> window.Buffer
 * 
 * @param  {String} module - module to require as global
 * @param  {String} opts   - if String, considered as `opts.as`
 * @param  {String} opts.as         - global variable name
 * @param  {String} opts.sub        - a submodule of the module as global
 * @param  {String} opts.expression - custom global declaration expression
 */
Wrap.prototype.global = function(module, opts) {
    //Buffer special shortcut
    if(module === 'Buffer' && !opts) {
        module = 'buffer';
        opts = {sub : 'Buffer'}
    }

    opts = (typeof opts === 'string') ? {as : opts} : opts;
    opts = opts || {};
    
    if(module) this.require(module);

    opts.as = opts.as || opts.sub || module;

    if(!opts.expression) {
        opts.expression = opts.as+' = require("'+ module +'")';
        opts.expression = (opts.sub) ? opts.expression +'.'+ opts.sub : opts.expression;
        opts.expression = opts.expression +';';
    }

    this.globals[opts.as] = opts.expression;
