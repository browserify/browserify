...
function makeTarget (file, root) {
    ...
    if ( isPrefixOf( path.join(root + '/'), file ) ) {
	return path.normalize('/' + file.slice(root.length + 1)).replace(/\\/g, '/');
    }
    else if (resolve.isCore(file) || !file.match(/([A-Za-z]:)?\\/)) {
	return path.normalize(file).replace(/\\/g, '/');
    }
    else if (s === nm && /^\/[^\/]+-browserify/.test(s) // Suspect this one, and below, regexp needs to be adjusted too in order to work in windows / with windows paths
    && file.slice(s.length), /^[^\/]+-browserify/.test(file.slice(s.length))) { // See above comment
	return file.slice(base.length).replace(/\\/g, '/');
    }
    else if (file.match(/[\/\\]node_modules[\/\\].+/)) {
	return path.normalize(file.match(/.*([\/\\]node_modules[\/\\].+)/)[1]).replace(/\\/g, '/');
    }
    else {
...
Wrap.prototype.require = function (mfile, opts) {
    ...
    var pkgfile = path.join( opts.basedir, '/package.json' );
    if (!mfile.match(/^(\.\.?)?\//)) {
        try {
