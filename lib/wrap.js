...
Wrap.prototype.wrap = function (target, body) {
		...
		var fname = ((dirname == '.' ? '' : dirname.replace(/\//g, '_')+'_')+path.basename(target).replace(/\./g, '_')+'_function').replace(/[^a-zA-Z0-9_]/, '_');

    return wrappers.body
      .replace(/\$__fname/g, function () {
          return fname;
      })
      .replace(/\$__filename/g, function () {
