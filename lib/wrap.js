...
Wrap.prototype.noParse = function (files) {
    if (!files) files = [];
    if (!Array.isArray(files)) files = files.split(',');

    this.noParsing = files.reduce(function (acc,x) {
        acc[x] = true;
        return acc;
    }, this.noParsing)
...
Wrap.prototype.require = function (mfile, opts) {
    ...
    try {
        var required
        if (! self.noParsing[mfile]) {
            required = self.detective.find(body);
        } else {
            required = {strings:[], expressions:[]};
        }
    }
    catch (err) {
        process.nextTick(function () {
