...
var Package = module.exports = function (pkgname, basedir, topPkg, opts) {
    ...
    if (!topPkg) {
        ...
        if (typeof base === 'string') {
            ...
            if (topPkg.dependencies) {
                return Package.merge(
                    Object.keys(topPkg.dependencies)
                        .filter(function (dep) {
                            return path.existsSync(
                                basedir + '/node_modules/' + dep
                            )
                        })
                        .map(function (dep) {
console.log(basedir + '/node_modules/' + dep);
                            return Package(
                                dep,
                                basedir + '/node_modules/' + dep,
                                null,
        ...
        .filter(function (file) {
            ...
            if (file.indexOf(basedir + '/node_modules/') === 0) {
                ...
                var inDev = Object.keys(ppkg.devDependencies).some(function (d) {
                    return file.indexOf(basedir + '/node_modules/' + d) === 0
                        && !ppkg.dependencies[d] // could mention a dep twice
                    ;
                });
                if (inDev) return false;
                
                var inUnmentionedDep = false;
                var orig = ppkg._original;
                if (orig.dependencies
                || (orig.browserify && orig.browserify.dependencies)) {
                    inUnmentionedDep = !Object.keys(ppkg.dependencies)
                        .some(function (d) {
                            return file.indexOf(
            ...
            else if (typeof pkg._original.browserify === 'object') {
                var base = pkg._original.base;
                if (typeof base === 'string') {
                    var isInBase = file.indexOf(base) === 0
                    return isInBase;
                }
            }
            else if (typeof pkg._original.browserify === 'string') {
                return file === path.resolve(basedir, pkg._original.browserify);
            }
