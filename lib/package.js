...
function normalizePkg (pkg, dir, opts) {
    ...
    if (p.browserify.dependencies) p.dependencies = p.browserify.dependencies;
    ...
    var srcFiles = files
        .filter(function (file) {
            ...
            if (file.indexOf(basedir + '/node_modules/') === 0) {
                ...
                var inDev = Object.keys(pkg.devDependencies).some(function (d) {
                    return file.indexOf(basedir + '/node_modules/' + d) === 0
                        && !pkg.dependencies[d] // could mention a dep twice
                    ;
                });
                if (inDev) return false;
            
                var inUnmentionedDep = false;
                var orig = pkg._original;
                if (orig.dependencies
                || (orig.browserify && orig.browserify.dependencies)) {
                    inUnmentionedDep = !Object.keys(pkg.dependencies)
                        .some(function (d) {
                            return file.indexOf(
